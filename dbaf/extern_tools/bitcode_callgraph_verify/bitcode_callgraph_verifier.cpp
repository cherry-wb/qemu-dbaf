...
public:
    static char ID;
    CallGraphPass() : ModulePass(ID), currentFunc(0),deepestFunc(0),curdepth(0), maxdepth(0) {
...
static RegisterPass<CallGraphPass> X("PCGP", " Call Graph Pass");

void CallGraphPass::visitNode(CallGraphNode *node){
    curdepth++;
...
void CallGraphPass::getInstructions(Function *F){
...
void CallGraphPass::printReport(Module &M){
...
int main(int argc, char **argv){
    ...
    Pass *P = static_cast<Pass*>(new CallGraphPass());
    PM->add(P);
    PM->run(*Mod);
    delete PM;
