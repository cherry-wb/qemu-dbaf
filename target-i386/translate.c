...
typedef struct DisasContext {
    ...
    int done_block_end; /* 1 when onTranslateInstructionEnd was called */
...
static inline void gen_instr_end(DisasContext *s,int forcemark)
{
    if (!s->done_instr_end) {
    	if(s->useNextPc)
		{
    		tcg_gen_movi_tl(cpu_tmpnextpc, s->nextPc);
		}
    	else
		{
    		tcg_gen_ld_tl(cpu_tmpnextpc, cpu_env, offsetof(CPUX86State, eip));
    		//tcg_gen_movi_tl(cpu_tmpnextpc, ((target_long)-1));
		}
        dbaf_on_translate_instruction_end(s->archcpuState,cpu_env,g_dbaf, g_dbaf_state, s->tb, s->insPc, cpu_tmpnextpc);
        s->done_instr_end = forcemark > 0 ? 1 : 0;
    }
}

#define SET_TB_TYPE(t) s->tb->dbaf_tb_type = t
#else
#define SET_TB_TYPE(t)
#define dbaf_on_translate_jump_start(...)
#endif
...
static inline void gen_goto_tb(DisasContext *s, int tb_num, target_ulong eip)
{
    ...
    if ((pc & TARGET_PAGE_MASK) == (tb->pc & TARGET_PAGE_MASK) ||
		...
		dbaf_on_translate_block_end(s->archcpuState,cpu_env,g_dbaf, g_dbaf_state,
									   s->tb, s->insPc, s->useNextPc, s->nextPc, cpu_tmpnextpc);
		s->done_block_end = 1; //one instrustion can have mutil goto opc, so we just mark it.
                               ...
                               target_ulong pc_start)
{
	...
	tcg_gen_movi_tl(cpu_tmpnextpc, (target_long)-1);
	dbaf_on_translate_instruction_start(env,cpu_env, g_dbaf, g_dbaf_state, s->tb, pc_start, cpu_tmpnextpc);
	s->tb->pcOfLastInstr = pc_start;
#endif
    s->pc = pc_start;
    prefixes = 0;
    s->override = -1;
 ...
 reswitch:
    ...
    case 0x13f: /* dbaf_op */
        {
            ...
            gen_helper_dbaf_tcg_custom_instruction_handler(cpu_env,tcg_const_i64(arg));
#else
            /* Simply skip the opcodes when building vanilla qemu */
    ...
    case 0xff: /* GRP5 */
        ...
        case 2: /* call Ev */
            ...
            gen_op_jmp_v(s, cpu_T[0],(target_ulong) -1);
            gen_eob(s);
            break;
        case 3: /* lcall Ev */
        	SET_TB_TYPE(TB_CALL_IND);
        ...
        do_ljmp:
            if (s->pe && !s->vm86) {
                ...
                gen_op_jmp_v(s,cpu_T[1],(target_ulong) -1);
            }
            gen_eob(s);
            break;
        case 6: /* push Ev */
    ...
    case 0xc2: /* ret im */
    	SET_TB_TYPE(TB_RET);
    	dbaf_on_translate_jump_start(env,cpu_env,g_dbaf, g_dbaf_state, s->tb, s->pc, JT_RET, cpu_tmpnextpc);
    ...
    case 0xca: /* lret im */
    	SET_TB_TYPE(TB_RET);
    	dbaf_on_translate_jump_start(env,cpu_env,g_dbaf, g_dbaf_state, s->tb, s->pc, JT_LRET, cpu_tmpnextpc);
        val = cpu_ldsw_code(env, s->pc);
        s->pc += 2;
    do_lret:
        if (s->pe && !s->vm86) {
                                                  ...
                                                  bool search_pc)
{
    ...
    dc->done_block_end = 0;
    dc->archcpuState = env;
    tb->dbaf_tb_type = TB_DEFAULT;
    tb->dbaf_tb_current_ins_type = INS_DEFAULT;
    tcg_gen_movi_tl(cpu_tmpnextpc, (target_long)-1);
    dbaf_on_translate_block_start(env,cpu_env,g_dbaf, g_dbaf_state, tb, pc_start, cpu_tmpnextpc);
#endif
    gen_tb_start();
    for(;;) {
        ...
        dc->useNextPc = 0;
        dc->nextPc = -1;
        dc->ins_opc = tcg_ctx.gen_opc_ptr;
        dc->ins_arg = tcg_ctx.gen_opparam_ptr;
#endif
