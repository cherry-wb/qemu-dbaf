...
void rr_debug_log_prog_point(RR_prog_point pp) {
	if(qemu_logfile)
		rr_spit_prog_point_fp(qemu_logfile,pp);
}
...
// our debug rr_assert
void rr_assert_fail(const char *exp, const char *file, int line, const char *function) {
    ...
    if(rr_debug_whisper() && qemu_logfile) {
        fprintf(qemu_logfile, "RR rr_assertion `%s' failed at %s:%d in %s\n", exp, file, line, function);
    }
    if(qemu_logfile)
    	fflush(qemu_logfile);
    // just abort
...
extern void *cpu_physical_memory_map(hwaddr addr, hwaddr *plen, int is_write);
extern void cpu_physical_memory_unmap(void *buffer, hwaddr len, int is_write, hwaddr access_len);
*/
...
void rr_create_record_log (const char *filename) {
  ...
  if (rr_debug_whisper() && qemu_logfile) {
    fprintf (qemu_logfile, "opened %s for write.\n", rr_nondet_log->name);
  }
...
void rr_create_replay_log (const char *filename) {
  ...
  if (rr_debug_whisper() && qemu_logfile) {
    fprintf (qemu_logfile, "opened %s for read.  len=%llu bytes.\n",
...
int rr_do_begin_record(const char *file_name_full, void *cpu_state) {
 ...
 char name_buf[1024];
  ...
  int snapshot_ret;
  if (rr_debug_whisper() && qemu_logfile) {
    fprintf (qemu_logfile,"Begin vm record for file_name_full = %s\n", file_name_full);
    fprintf (qemu_logfile,"path = [%s]  file_name_base = [%s]\n", rr_path, rr_name);
  }
  ...
  if (rr_record_requested == 2) {
    ...
    g_free((gpointer)rr_snapshot_name);
  ...
  if (rr_record_requested  == 1 || rr_record_requested == 2) {
    ...
    snapshot_ret = do_savevm_rr(default_mon, name_buf);
    log_all_cpu_states();
  }
...
void rr_do_end_record(void) {
  ...
  if (rr_debug_whisper() && qemu_logfile) {
    fprintf (qemu_logfile,"End vm record for name = %s\n", rr_name);
...
int rr_do_begin_replay(const char *file_name_full, void *cpu_state) {
  ...
  if (rr_debug_whisper() && qemu_logfile) {
    fprintf (qemu_logfile,"Begin vm replay for file_name_full = %s\n", file_name_full);
    fprintf (qemu_logfile,"path = [%s]  file_name_base = [%s]\n", rr_path, rr_name);
  }
  // first retrieve snapshot
  rr_get_snapshot_file_name(rr_name, rr_path, name_buf, sizeof(name_buf));
  if (rr_debug_whisper() && qemu_logfile) {
    fprintf (qemu_logfile,"reading snapshot:\t%s\n", name_buf);
  }
