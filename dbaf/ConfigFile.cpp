...
int ConfigFile::RegisterDBAFApi()
{
    Lunar<DBAFLUAExecutionState>::Register(m_luaState);
    return 0;
}
...
void ConfigFile::luaError(const char *fmt, ...)
{
    ...
    if (g_dbaf) {
        ...
        g_dbaf->getErrorStream() << "ERROR: " << str << '\n';
    }else {
...
void ConfigFile::luaWarning(const char *fmt, ...)
{
    ...
    if (g_dbaf) {
        char str[512];
        vsnprintf(str, sizeof(str), fmt, v);
        g_dbaf->getWarningStream() << "WARNING: " << str << '\n';
    }else {
        vfprintf(stderr, fmt, v);
    }
    va_end(v);
...
//////////////////////////////////////////////////
const char DBAFLUAExecutionState::className[] = "DBAFLUAExecutionState";

Lunar<DBAFLUAExecutionState>::RegType DBAFLUAExecutionState::methods[] = {
  LUNAR_DECLARE_METHOD(DBAFLUAExecutionState, readMemory),
  LUNAR_DECLARE_METHOD(DBAFLUAExecutionState, writeMemory),
  {0,0}
};

DBAFLUAExecutionState::DBAFLUAExecutionState()
{
	if (g_dbaf)
		g_dbaf->getDebugStream() << "Creating DBAFLUAExecutionState" << '\n';
}
DBAFLUAExecutionState::DBAFLUAExecutionState(lua_State *L)
{
	if (g_dbaf)
		g_dbaf->getDebugStream() << "Creating DBAFLUAExecutionState" << '\n';
}
...
{
	if (g_dbaf)
		g_dbaf->getDebugStream() << "Deleting DBAFLUAExecutionState" << '\n';
}


int DBAFLUAExecutionState::readMemory(lua_State *L)
{
//    target_ulong address = luaL_checkint(L, 1);
//    uint32_t size = luaL_checkint(L, 2);
	if (g_dbaf)
		g_dbaf->getDebugStream() << "DBAFLUAExecutionState readMemory" << '\n';
    return 1;
}

int DBAFLUAExecutionState::writeMemory(lua_State *L)
{
//    target_ulong address = luaL_checkint(L, 1);
//    uint32_t size = luaL_checkint(L, 2);
//    target_ulong value = luaL_checkint(L, 3);
	if (g_dbaf)
		g_dbaf->getDebugStream() << "DBAFLUAExecutionState writeMemory" << '\n';
    return 0;
}
